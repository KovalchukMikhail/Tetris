...
namespace Tetris
{
    //Класс реализующий левый L блок
    public class BlockLLeft
    {
        ...
        public BlockLLeft(int initialX, int initialY) //конструктор для элемента который появляется в начале раунда
        {
            ...
            currentPosition[initialX, initialY + 1] = 1;
            currentPosition[initialX, initialY] = 2;
            currentPosition[initialX + 1, initialY] = 3;
            currentPosition[initialX + 2, initialY] = 4;
...
//В массивах хранящих адрес каждого элемента в массиве currentPosition актуализируются значения
        ...
        public void MoveChange() // метод реализующий поворот фигуры. вначале проверяет возможен ли поворот а потом все аналогично комментариям к движению вниз
        {
            if (tipe == 0)
            {
                if (fourthPixelPosition[1] + 2 < currentPosition.GetLength(1))
                {
                    currentPosition[firstPixelPosition[0], firstPixelPosition[1]] = 0;
                    currentPosition[secondPixelPosition[0], secondPixelPosition[1]] = 0;
                    currentPosition[thirdPixelPosition[0], thirdPixelPosition[1]] = 0;
                    ...
                    currentPosition[firstPixelPosition[0] + 1, firstPixelPosition[1] - 1] = 1;
                    currentPosition[secondPixelPosition[0] + 2, secondPixelPosition[1]] = 2;
                    currentPosition[thirdPixelPosition[0] + 1, thirdPixelPosition[1] + 1] = 3;
                    currentPosition[fourthPixelPosition[0], fourthPixelPosition[1] + 2] = 4;
            ...
            else if (tipe == 1)
            {
                if (fourthPixelPosition[0] - 2 >= 0)
                {
                    ...
                    currentPosition[firstPixelPosition[0] + 1, firstPixelPosition[1] + 1] = 1;
                    currentPosition[secondPixelPosition[0], secondPixelPosition[1] + 2] = 2;
                    currentPosition[thirdPixelPosition[0] - 1, thirdPixelPosition[1] + 1] = 3;
                    currentPosition[fourthPixelPosition[0] - 2, fourthPixelPosition[1]] = 4;
            ...
            {
                if (fourthPixelPosition[0] + 2 < currentPosition.GetLength(0))
                {
                    currentPosition[firstPixelPosition[0], firstPixelPosition[1]] = 0;
                    currentPosition[secondPixelPosition[0], secondPixelPosition[1]] = 0;
                    currentPosition[thirdPixelPosition[0], thirdPixelPosition[1]] = 0;
